...
func Run(fn func(gen *Plugin)) error {
	var p = &Plugin{files: make([]*compiler, 0)}
	fn(p)
	return p.output()
}
...
func (p *Plugin) NewFile(name string, path string, exts ...*Package) *compiler {
	...
	f.imports(exts...)
	f.Println("// Code generated by https://github.com/charlesbases/reverse. DO NOT EDIT.")
	f.Println("// date: ", time.Now().Format(rfc))

	return f
}

// Import .
func (f *compiler) Import(exts ...*Package) {
	f.imports(exts...)
}

// Println .
func (f *compiler) Println(v ...interface{}) {
	if len(v) != 0 {
		fmt.Fprint(f.buf, v...)
	}
	fmt.Fprintln(f.buf)
...
// Printlnf formats according to a format specifier and writes to file.
func (f *compiler) Printlnf(format string, v ...interface{}) {
	fmt.Fprintf(f.buf, format, v...)
	fmt.Fprintln(f.buf)
}

// imports .
func (f *compiler) imports(exts ...*Package) {
...
// content .
func (f *compiler) content() ([]byte, error) {
